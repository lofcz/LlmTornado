...
namespace OpenAiNg.Chat
{
	...
	public class ChatEndpoint : EndpointBase, IChatEndpoint
	{
		...
		public IAsyncEnumerable<ChatResult> StreamChatEnumerableAsync(ChatRequest request)
		{
			...
			if (request.OuboundFunctionsContent is not null)
			{
				request.OuboundFunctionsContent.Ptr = request.Functions is null ? string.Empty : JsonConvert.SerializeObject(request.Functions, NullSettings);
			}
