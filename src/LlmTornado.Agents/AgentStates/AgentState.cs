...
namespace LlmTornado.Agents.AgentStates
{
    ...
    public abstract class AgentState<TInput, TOutput> : BaseState<TInput, TOutput>, IAgentState
    {
        ...
        public abstract TornadoAgent InitilizeStateAgent();
        ...
        public AgentState(StateMachine stateMachine)
        {
            ...
            StateAgent = InitilizeStateAgent(); // Initialize the agent state, which sets up the agent and its properties
        ...
        /// </summary>
        /// <remarks>Use this to recieve the first text from the Run, and processes with another agent other than the state Agent</remarks>
        ...
        {
            return (await TornadoRunner.RunAsync(agent, input, verboseCallback: RunnerVerboseCallbacks,streamingCallback:StreamingCallbacks ,streaming: streaming, cancellationToken: CancelTokenSource));
        }
        ...
        {
            return (await TornadoRunner.RunAsync(StateAgent, input, verboseCallback: RunnerVerboseCallbacks, streamingCallback: StreamingCallbacks, streaming: streaming, cancellationToken: CancelTokenSource));
        }
        ...
        public async Task<T> BeginRunnerAsync<T>(string input, bool streaming = false, int maxRetries = 2)
        {
            ...
            {
                var result = await TornadoRunner.RunAsync(StateAgent, input,
                ...
                if (result.Messages.Last().Content.TryParseJson<T>(out var parsedResult))
                {
                    return parsedResult;
                }

                // Attempt JSON repair if standard parsing fails
                var repairedJson = await RepairJsonAsync(result.Messages.Last().Content, typeof(T));
        ...
        private async Task<string> RepairJsonAsync(string possibleJson, Type targetType)
        {
            try
            {
                ...
                var repairResult = await TornadoRunner.RunAsync(StateAgent, repairPrompt,
                    cancellationToken: CancelTokenSource);
