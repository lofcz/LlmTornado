...
{
    QdrantVectorDatabase qdrantClient;

    [SetUp]
    public async Task Setup()
    {
        qdrantClient = new QdrantVectorDatabase("localhost", port: 6334, vectorDimension: 1536);
        await qdrantClient.InitializeCollectionAsync("golden-data");
    }

    [Test]
    public async Task CreateCollectionTest()
    {
        Assert.That(qdrantClient.GetCollectionName(), Is.EqualTo("golden-data"));
    }

    [Test]
    public async Task AddDocsQdrantTest()
    {
        List<VectorDocument> vectorDocuments = new List<VectorDocument>();
        var json = File.ReadAllText("Static/Files/golden_data.json");
        var faissData = Newtonsoft.Json.JsonConvert.DeserializeObject<VectorDbCollection>(json);
        ...
        await qdrantClient.UpsertDocumentsAsync(vectorDocuments.ToArray());
    }

    [Test]
    public async Task QueryQdrantTest()
    {
        // Example embedding to query
        ...
        var results = await qdrantClient.QueryByEmbeddingAsync(embedding, topK: 1, includeScore:true);
        foreach (var doc in results)
        {
            Console.WriteLine($"Id: {doc.Id}, Score: {doc.Score}");
            Console.WriteLine($"Content: {doc.Content.Substring(0, Math.Min(100, doc.Content.Length))}...");
            Console.WriteLine("-----");
    ...
    [Test]
    public async Task GetQdrantDocTest()
    {   
        var docs = await qdrantClient.GetDocumentsAsync(new[] { "3d238da5-ee32-4397-941d-0bc904e760d4" });
    ...
    [Test]
    public async Task GetQdrantDocWhereTest()
    {
        var docs = await qdrantClient.GetDocumentWhere(TornadoWhereOperator.Equal("page",8));
        foreach (var doc in docs)
        {
            Console.WriteLine($"Id: {doc.Id}");
            Console.WriteLine($"Content: {doc.Content.Substring(0, Math.Min(100, doc.Content.Length))}...");
            Console.Write($"Metadata: {string.Join(",", doc.Metadata.Select(kvp => $"{kvp.Key}: {kvp.Value}"))}");
            Console.WriteLine("-----");
        }
    }

    [Test]
    public async Task CloudQDrantTest()
    {
        QdrantVectorDatabase cloudQdrant = new QdrantVectorDatabase(
            "dont.include.https.us-east4-0.gcp.cloud.qdrant.io",
            apiKey: "database-api-key",
            https: true,
            vectorDimension: 1536);

        await cloudQdrant.InitializeCollectionAsync("tornado-test-collection");

        var collections = await cloudQdrant.GetCollectionList();
        ...
        var embedding = await api.Embeddings.CreateEmbedding(EmbeddingModel.OpenAi.Gen3.Small, "Sample document for Qdrant Cloud");
        VectorDocument doc = new VectorDocument(Guid.NewGuid().ToString(), "Sample document for Qdrant Cloud")
        {
            Embedding = embedding,
            Metadata = new Dictionary<string, object> { { "source", "unit-test" } }
        };

        await cloudQdrant.UpsertDocumentsAsync(new[] { doc });
        // Query the document back
        var results = await cloudQdrant.QueryByEmbeddingAsync(embedding, topK: 1, includeScore: true);
        foreach (var result in results)
        {
            Console.WriteLine($"Id: {result.Id}, Score: {result.Score}");
            Console.WriteLine($"Content: {result.Content}");
            Console.WriteLine("-----");
            Assert.That(result.Id.Equals(doc.Id));
        }

        await cloudQdrant.DeleteCollectionAsync("tornado-test-collection");
