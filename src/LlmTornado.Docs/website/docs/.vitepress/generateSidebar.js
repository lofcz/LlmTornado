...
export function generateSidebar() {
  ...
  const section = text => autoGeneratedItems.find(item => item.text === text)
  const core = section('LlmTornado')
  const coreItem = text => core?.items?.find(item => item.text === text)
  const compact = items => items.filter(Boolean)

  const legacy = compact([
    coreItem('Assistants'),
    coreItem('Threads'),
    coreItem('Completions')
  ]).map(item => ({ ...item, text: `${item.text} (Legacy)` }))

  const primaryCore = core?.items?.filter(item =>
    !['Anthropic Specific', 'Assistants', 'Threads', 'Completions'].includes(item.text)
  ) ?? []

  return [
    { text: 'Getting Started', link: '/getting-started' },
    { text: 'Core API', collapsed: false, items: primaryCore },
    section('Agents'),
    {
      text: 'Integrations',
      collapsed: true,
      items: compact([
        section('MPC') ? { ...section('MPC'), text: 'MCP' } : null,
        section('Microsoft.Extension.AI'),
        section('VectorDatabases')
      ])
    },
    section('Tutorials'),
    {
      ...
      collapsed: true,
      items: compact([
        ...(section('Advanced and Preview')?.items ?? []),
        section('A2A'),
        coreItem('Anthropic Specific')
      ])
    },
    { text: 'Legacy APIs', collapsed: true, items: legacy }
  ].filter(Boolean)
}
