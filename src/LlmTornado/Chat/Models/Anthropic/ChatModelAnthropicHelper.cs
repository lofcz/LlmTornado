...
public static class ChatModelAnthropicHelper
{
    ...
    /// <summary>
    /// Claude Opus 4.7+ reject non-default <c>temperature</c>, <c>top_p</c>, and <c>top_k</c> with HTTP 400.
    /// </summary>
    public static bool RejectsNonDefaultSamplingParams(string? modelName) => IsOpus47OrNewer(modelName);

    /// <summary>
    /// Claude Opus 4.7+ reject manual extended thinking (<c>thinking.type = enabled</c> with <c>budget_tokens</c>).
    /// </summary>
    public static bool RejectsManualExtendedThinking(string? modelName) => IsOpus47OrNewer(modelName);

    /// <summary>
    /// Claude Opus 4.7+ only support adaptive thinking when thinking is enabled.
    /// </summary>
    public static bool RequiresAdaptiveThinkingWhenEnabled(string? modelName) => IsOpus47OrNewer(modelName);

    /// <summary>
    /// High-resolution vision (up to 2576px long edge) is automatic on Claude Opus 4.7+.
    /// </summary>
    public static bool SupportsHighResVision(string? modelName) => IsOpus47OrNewer(modelName);

    /// <summary>
    /// Known Opus 4.7+ chat models for policy checks keyed by <see cref="IModel.Name"/>.
    /// </summary>
