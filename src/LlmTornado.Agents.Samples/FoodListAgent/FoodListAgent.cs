...
public class FoodListAgentConfiguration : OrchestrationRuntimeConfiguration
{
    FoodPlannerRunnable planner;
    FoodResearchRunnable researcher;
    FoodReportingRunnable reporter;
    ExitRunnable exit;
    public int MaxDepth { get; set; } = 3;
    public int MinWordCount { get; set; } = 250;
    public TornadoApi Client { get; set; }

    public FoodListAgentConfiguration()
    {
        Client = new TornadoApi(Environment.GetEnvironmentVariable("OPENAI_API_KEY"), LLmProviders.OpenAi);
        RecordSteps = true;
        //Create the Runnables
        planner = new FoodPlannerRunnable(Client, this, MaxDepth);
        researcher = new FoodResearchRunnable(Client, this, MaxDepth);
        reporter = new FoodReportingRunnable(Client, this, MinWordCount);
    ...
    public FoodListAgentConfiguration(TornadoApi? client = null)
    {
...
public class FoodPlannerRunnable : OrchestrationRunnable<ChatMessage, WebSearchPlan>
{
    TornadoAgent Agent;
    private int _maxItemsToPlan = 10;
    public FoodPlannerRunnable(TornadoApi client, Orchestration orchestrator, int maxItemsToPlan) : base(orchestrator)
    {
...
public class FoodResearchRunnable : OrchestrationRunnable<WebSearchPlan, string>
{
    public int MaxDegreeOfParallelism { get; set; } = 4;
    public int MaxItemsToProcess { get; set; } = 3;
    TornadoApi Client { get; set; }
    public FoodResearchRunnable(TornadoApi client, Orchestration orchestrator, int maxItemsToProcess = 3) : base(orchestrator) { Client = client; MaxItemsToProcess = maxItemsToProcess; }
...
public class FoodReportingRunnable : OrchestrationRunnable<string, ReportData>
{
    TornadoAgent Agent;

    public Action<AgentRunnerEvents>? OnAgentRunnerEvent { get; set; }
    private int _minWordCount { get; set; } = 200;
    public FoodReportingRunnable(TornadoApi client, Orchestration orchestrator, int minWordCount) : base(orchestrator)
    {
        string instructions = $"""
